...
let benchmark_time (cfg : Config.t) test n : unit t =
...
let benchmark_not_time (cfg : Config.t) : unit t =
...
let () =
  ...
  (* let s = cfg |> Config.sexp_of_t |> Sexp.to_string_hum in
     print_endline s ; *)
  let testcases = cfg.testcases_to_time in
  ignore
  @@ eval
