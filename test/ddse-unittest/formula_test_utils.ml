...
let assert_solutions constraint_list solutions =
  ...
  | Some (model, _) ->
      solutions
      |> List.iter (fun (symbol, expected_answer) ->
             let actual_answer = model symbol in
             if not @@ Option.eq ~eq:equal_value expected_answer actual_answer
             then
               assert_failure
               @@
               let set_msg =
                 indent @@ string_of_constraint_list constraint_list
               in
               Printf.sprintf
                 "Expected %s but got %s for symbol %s in constraints:\n%s"
                 (Option.default "<none>"
                 @@ Option.map show_value expected_answer)
                 (Option.default "<none>" @@ Option.map show_value actual_answer)
                 (show_symbol symbol) set_msg)
