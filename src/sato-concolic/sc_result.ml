...
let get_odefa_errors (sato_state : Sc_state.t)
    (symb_interp_state : Types.State.t) (interp_session : Interpreter.session)
    (final_env : Interpreter.denv) : Ast.clause * Sc_error.Jayil_error.t list =
  ...
  let mk_value_error x x_stk =
    ...
    let value_error =
      Sc_error.Jayil_error.Error_value
...
type odefa_error_record = {
  err_errors : Sc_error.Jayil_error.t list;
...
module Jayil_type_errors : Sc_result with type t = odefa_error_record = struct
  type t = odefa_error_record

  let description = "error"

  let get_errors (sato_state : Sc_state.t)
    ...
    let odefa_inst_maps = sato_state.odefa_instrumentation_maps in
    let rm_inst_fn =
      Sc_error.jayil_error_remove_instrument_vars odefa_inst_maps
    in
    {
  ...
  let show : t -> string =
   fun error ->
    ...
    ^ "--------------------\n"
    ^ String.concat ~sep:"\n--------------------\n"
    @@ List.map ~f:Sc_error.Jayil_error.show error.err_errors
...
type natodefa_error_record = {
  err_errors : Sc_error.On_error.t list;
  err_input_seq : int option list;
  err_location : Jay_error_location.t;
}
[@@deriving to_yojson]

module Jay_type_errors : Sc_result with type t = natodefa_error_record = struct
  ...
  let get_errors (sato_state : Sc_state.t)
    ...
    let on_err_list =
      let mapper =
        Sc_error.jayil_to_jay_error odefa_inst_maps on_to_odefa_maps
  ...
  let show : t -> string =
   fun error ->
    ...
    ^ "--------------------\n"
    ^ String.concat ~sep:"\n--------------------\n"
    @@ List.map ~f:Sc_error.On_error.show error.err_errors
...
type ton_error_record = {
  err_errors : Sc_error.Bluejay_error.t list;
  err_input_seq : int option list;
  err_location : Bluejay_error_location.t;
}
[@@deriving to_yojson]

module Bluejay_type_errors : Sc_result with type t = ton_error_record = struct
  ...
  let get_errors (sato_state : Sc_state.t)
    ...
    let ton_err_list =
      let mapper =
        if is_type_error
        then
          Sc_error.jayil_to_bluejay_error odefa_inst_maps on_to_odefa_maps
            ton_on_maps interp_session on_err_loc_syn
        else
          Sc_error.jayil_to_bluejay_error_simple odefa_inst_maps
  ...
  let show : t -> string =
   fun error ->
    ...
    ^ "--------------------\n"
    ^ String.concat ~sep:"\n--------------------\n"
    @@ List.map ~f:Sc_error.Bluejay_error.show error.err_errors
