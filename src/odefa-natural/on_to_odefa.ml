...
(** In this module we will translate from odefa-natural to odefa in the
    ...
    * Alphatize program * Annotate recursive call sites * Desugar let rec *
    Desugar lists * Desugar variants * Desugar pattern vars * Desugar pattern
    matching * Alphatize program again (to allow above transformations to
    introduce dupes) *)
...
let flatten_fun ?(binding_name = (None : On_ast.Ident.t option))
    (param_names : On_ast.Ident.t list) (body : Ast.expr) :
    (Ast.expr * Ast.var) m =
  list_fold_right_m
...
let rec pat_rename_vars (renaming : On_ast.Ident.t On_ast.Ident_map.t)
    (pat : On_ast.pattern) : On_ast.pattern =
  match pat with
  | On_ast.AnyPat -> pat
  | On_ast.IntPat -> pat
  | On_ast.TruePat -> pat
  ...
  | On_ast.VarPat i ->
      On_ast.VarPat (On_ast.Ident_map.find_default i i renaming)
  | On_ast.FunPat -> pat
  | On_ast.EmptyLstPat -> pat
  | On_ast.LstDestructPat (hd, tl) ->
      On_ast.LstDestructPat
        (pat_rename_vars renaming hd, pat_rename_vars renaming tl)
  ...
  let transformer recurse e =
    ...
    | On_ast.LetRecFun (fun_sig_list, rec_expr) ->
        ...
        let original_names =
          List.map
            (fun single_sig ->
              let (On_ast.Funsig (id, _, _)) = single_sig in
              id)
            fun_sig_list
        in
        let%bind new_names =
            ...
            (fun appl_dict base_fun ->
              ...
              let sub_appl =
                List.fold_left
                  ...
                  (Var new_fun_name) new_names
              in
              return
              @@ On_ast.Ident_map.add original_fun_name sub_appl appl_dict)
            On_ast.Ident_map.empty name_pairs
        in
        let let_maker_fun fun_name acc =
        ...
        let%bind ret_expr =
          list_fold_right_m
            (fun (fun_sig, fun_new_name) acc ->
              ...
              let new_fun_expr =
                List.fold_right let_maker_fun original_names
                  transformed_cur_f_expr
              in
              return
              @@ On_ast.Let
                   (fun_new_name, Function (new_param_list, new_fun_expr), acc))
...
let rec rename_variable (old_name : On_ast.ident) (new_name : On_ast.ident)
  ...
  | On_ast.Appl (e1, e2) -> On_ast.Appl (recurse e1, recurse e2)
  | On_ast.Let (id, e1, e2) ->
  ...
  | On_ast.LetRecFun (f_sigs, e') ->
      ...
      let f_sigs' =
        if On_ast.Ident_set.mem old_name function_names then
          ...
          f_sigs
          |> List.map (fun (On_ast.Funsig (name, params, body)) ->
  ...
  | On_ast.LetFun (f_sig, e') ->
      ...
      if id = old_name then
        ...
        *)
        List.exists (On_ast.Ident.equal old_name) id_list
      then
        let new_e' = recurse e' in
...
let alphatize (e : On_ast.expr) : On_ast.expr m =
  ...
  let rec ensure_exprs_unique_names (names : Ident.t list) (exprs : expr list)
    ...
    | name :: more_names ->
        ...
        in
        if Ident_set.mem name previously_declared' then
          let (Ident s) = name in
  ...
  let rec walk (expr : expr) (seen_declared : Ident_set.t) :
    ...
    | LetRecFun (funsigs, expr) ->
        ...
        let function_names, function_bodies =
          List.split
          @@ List.map (fun (Funsig (name, _, body)) -> (name, body)) funsigs'
        in
...
let rec flatten_binop (e1 : On_ast.expr) (e2 : On_ast.expr)
  ...
  let new_clause =
    Ast.Clause (new_var, Ast.Binary_operation_body (e1_var, binop, e2_var))
  in
  return (e1_clist @ e2_clist @ [ new_clause ], new_var)

and flatten_expr (e : On_ast.expr) : (Ast.clause list * Ast.var) m =
  match e with
  | Var id ->
  ...
  | Appl (e1, e2) ->
      ...
      let new_clause = Ast.Clause (new_var, Ast.Appl_body (e1_var, e2_var)) in
      return (e1_clist @ e2_clist @ [ new_clause ], new_var)
  | Let (var_ident, e1, e2) ->
      let%bind e1_clist, e1_var = flatten_expr e1 in
      let%bind e2_clist, e2_var = flatten_expr e2 in
  ...
  | Plus (e1, e2) -> flatten_binop e1 e2 Ast.Binary_operator_plus
  | Minus (e1, e2) -> flatten_binop e1 e2 Ast.Binary_operator_minus
  | Times (e1, e2) -> flatten_binop e1 e2 Ast.Binary_operator_times
  | Divide (e1, e2) -> flatten_binop e1 e2 Ast.Binary_operator_divide
  | Modulus (e1, e2) -> flatten_binop e1 e2 Ast.Binary_operator_modulus
  | Equal (e1, e2) -> flatten_binop e1 e2 Ast.Binary_operator_equal_to
  | LessThan (e1, e2) -> flatten_binop e1 e2 Ast.Binary_operator_less_than
  | Leq (e1, e2) ->
      flatten_binop e1 e2 Ast.Binary_operator_less_than_or_equal_to
  | And (e1, e2) -> flatten_binop e1 e2 Ast.Binary_operator_and
  | Or (e1, e2) -> flatten_binop e1 e2 Ast.Binary_operator_or
  | Not e ->
      let%bind e_clist, e_var = flatten_expr e in
      let%bind true_var = fresh_var "true" in
      let true_clause =
        Ast.Clause (true_var, Ast.Value_body (Value_bool true))
      in
      let%bind new_var = fresh_var "binop" in
      let new_clause =
        Ast.Clause
          ( new_var,
            Ast.Binary_operation_body (e_var, Ast.Binary_operator_xor, true_var)
          )
      in
  ...
  | If (e1, e2, e3) ->
      ...
      let new_clause =
        Ast.Clause
          ( new_clausevar,
            Ast.Conditional_body (e1_var, Expr e2_clist, Expr e3_clist) )
      in
  ...
  | Record recexpr_map ->
      ...
      let new_clause =
        Ast.Clause
          (rec_var, Ast.Value_body (Ast.Value_record (Ast.Record_value map)))
      in
  ...
  | Match (subject, pat_expr_list) ->
      ...
      let match_converter curr acc =
        ...
        let rec pat_conversion pat =
          ...
          | On_ast.RecPat patmap ->
              let rec_conversion key entry acc =
              ...
              in
              Record_pattern new_pat_map
          | On_ast.VariantPat _ ->
              raise
              @@ Utils.Invariant_failure
                   "match_converter: Variants patterns should have been \
                    encoded!"
          | On_ast.VarPat _ ->
              raise
              @@ Utils.Invariant_failure
        ...
        let if_clause =
          Ast.Clause
            (cond_var, Ast.Conditional_body (match_var, Expr flat_pat_expr, acc))
        in
      ...
      let%bind explode_expr =
        ...
        let zero_clause =
          Ast.Clause (zero_var, Ast.Value_body (Ast.Value_int 0))
        in
...
let debug_transform (name : string) (transform : On_ast.expr -> On_ast.expr m)
    (e : On_ast.expr) : On_ast.expr m =
  ( lazy_logger `trace @@ fun () ->
    Printf.sprintf "%s on:\n%s" name (On_ast.show_expr e) );
  let%bind answer = transform e in
  ( lazy_logger `trace @@ fun () ->
    Printf.sprintf "%s on:\n%s\nproduces\n%s" name (On_ast.show_expr e)
      (On_ast.show_expr answer) );
  return answer

let translate ?(translation_context = None) (e : On_ast.expr) :
    Odefa_ast.Ast.expr =
  let e_m =
