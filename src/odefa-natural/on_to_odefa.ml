...
and
  ...
  | Not (_) ->
    ...
    let new_clause =
      Ast.Clause(new_var,
                 Ast.Unary_operation_body(Ast.Unary_operator_not, e_var))
    in
    (e_clist @ [new_clause], new_var) *)
    failwith "no Not in ddse"
  | If (_, _, _) ->
    ...
    failwith "condition syntax change"
  | Int (n) ->
