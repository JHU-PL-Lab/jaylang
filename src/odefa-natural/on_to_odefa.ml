  ...
  let renamed_expr = 
    match e with
      ...
      | If (e1, e2, e3) -> 
        ...
        If (e1', e2', e3')
      | Record m -> 
        Record (Ident_map.map (fun ed -> recurse ed) m)
      | RecordProj (e1, lbl) -> 
        let e1' = recurse e1 in
        RecordProj (e1', lbl)
      | VariantExpr (lbl, e1) ->
        ...
        VariantExpr (lbl, e1')
      | List es -> 
        List (List.map (fun ed -> recurse ed) es)
      | ListCons (e1, e2) -> 
        let e1' = recurse e1 in
        let e2' = recurse e2 in
        ListCons (e1', e2')
...
let new_odefa_var 
  (e_desc : On_ast.expr_desc) (var_name : string) 
  : (Ast.var) m =
...
let add_abort_expr 
  (e_desc : On_ast.expr_desc) (_ : Ast.var list) : Ast.expr m =
...
and flatten_pattern_match
  ...
  let rec convert_match
      ((pat, e) : On_ast.pattern * On_ast.expr_desc)
...
and flatten_expr
  ...
  | LetFun (sign, e) ->
    ...
    let%bind (Expr(fun_clauses), return_var) =
      flatten_fun ~binding_name:(Some fun_name) expr_desc id_list (Expr(body_clist))
    in
    (* Flattening the "e2"... *)
...
let debug_transform_on
    (trans_name : string)
    (transform : 'a -> On_ast.expr_desc m)
    (e : 'a)
  : On_ast.expr_desc m =
