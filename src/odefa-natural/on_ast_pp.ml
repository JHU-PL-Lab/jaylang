...
and pp_binop formatter expr =
  ...
  | Appl (e1, e2) ->
      ...
      if l_cmp < 0 && r_cmp <= 0
      then
        Format.fprintf formatter "(%a) %a (%a)" pp_expr e1 pp_symb expr pp_expr
          e2
...
and pp_expr formatter expr =
  ...
  | Record record -> pp_ident_map pp_expr formatter record
  | List e_list ->
      Pp_utils.pp_concat_sep_delim "[" "]" ","
  ...
  | LetRecFun (funsig_lst, e) ->
      let pp_funsig_list formatter funsig_lst =
        Pp_utils.pp_concat_sep " with " pp_funsig formatter
          (List.enum funsig_lst)
      in
      Format.fprintf formatter "let rec@ %a@ in@ @[%a@]" pp_funsig_list
        funsig_lst pp_expr e
  | LetFun (funsig, e) ->
      Format.fprintf formatter "let@ %a@ in@ @[%a@]" pp_funsig funsig pp_expr e
  | If (pred, e1, e2) ->
      Format.fprintf formatter "if@ %a@ then@ @[<2>%a@]@ else @[<2>%a@]" pp_expr
        pred pp_expr e1 pp_expr e2
  | Match (e, pattern_expr_list) ->
      ...
      in
      Format.fprintf formatter "match@ %a@ with@ @[%a@]@ end" pp_expr e
  ...
  (* Unary operations *)
  | Not e ->
      if expr_precedence_cmp expr e < 0
      then Format.fprintf formatter "not %a" pp_expr e
      else Format.fprintf formatter "not (%a)" pp_expr e
  | RecordProj (e, lbl) ->
      if expr_precedence_cmp e expr > 0
      then Format.fprintf formatter "%a.%a" pp_expr e pp_label lbl
      else Format.fprintf formatter "(%a).%a" pp_expr e pp_label lbl
  | VariantExpr (variant_lbl, e) ->
      if expr_precedence_cmp expr e < 0
      then
        Format.fprintf formatter "%a %a" pp_variant_label variant_lbl pp_expr e
      else
        Format.fprintf formatter "%a (%a)" pp_variant_label variant_lbl pp_expr
          e
  | Assert e ->
      if expr_precedence_cmp expr e < 0
      then Format.fprintf formatter "assert %a" pp_expr e
      else Format.fprintf formatter "assert (%a)" pp_expr e
  | Assume e ->
      if expr_precedence_cmp expr e < 0
      then Format.fprintf formatter "assume %a" pp_expr e
      else Format.fprintf formatter "assume (%a)" pp_expr e
