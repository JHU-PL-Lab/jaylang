...
type lookup_environment = {
  le_cfg : ddpa_graph;  (** The DDPA CFG to use during lookup. *)
  ...
  le_clause_predecessor_mapping : clause Ident_map.t;
      (** A mapping from clauses (represented by the identifier they define) to
          the concrete clauses which appear immediately *before* them. The first
          clause in each expression does not appear in this map. *)
  le_function_parameter_mapping : function_value Ident_map.t;
      (** A mapping from function parameter variables to the functions which
          declare them. *)
  le_function_return_mapping : function_value Ident_map.t;
      (** A mapping from function return variables to the functions which
          declare them. *)
...
(** Given a program and its corresponding CFG, constructs an appropriate lookup
    environment. *)
let prepare_environment (e : expr) (cfg : ddpa_graph) : lookup_environment =
  ...
  let function_parameter_mapping, function_return_mapping =
    ...
    |> Enum.fold
         (fun (pm, rm) (Function_value (Var (p, _), Expr clss) as f) ->
           ...
           let retvar =
             clss |> List.last |> fun (Clause (Var (r, _), _)) -> r
           in
...
module Interpreter_cache_key :
  Symbolic_monad.Cache_key with type 'a t = 'a interpreter_cache_key = struct
    ...
    | ( Cache_lookup (lookup_stack_1, acl1, relative_stack_1),
        ...
        if c < 0 then
          ...
          if c < 0 then
            ...
            let c = Relative_stack.compare relative_stack_1 relative_stack_2 in
            if c < 0 then
              Lt
            else if c > 0 then
              Gt
    ...
    | Cache_lookup (lookup_stack, acl, relative_stack) ->
        Format.fprintf formatter "lookup(%s,%s,%s)"
          (Jhupllib.Pp_utils.pp_to_string
             (Jhupllib.Pp_utils.pp_list pp_ident)
             lookup_stack)
...
module Interpreter_cache_key_least_relative_stack_repetition_ordering = struct
  ...
  let reps (a : t) =
    ...
    | Some (Some_key (Cache_lookup (_, _, relstack))) ->
        ...
        in
        let repetitions =
          occurrences |> Ident_map.enum
          |> Enum.map (fun (_, v) -> if v < 2 then 0 else v - 1)
          |> Enum.sum
        in
        repetitions

  let compare (a : t) (b : t) : int = Stdlib.compare (reps a) (reps b)
end
...
module Make
    (C : Symbolic_monad.WorkCollection
           with module Work_cache_key = Interpreter_cache_key) : Interpreter =
struct
  ...
  *)
  ...
  let rec lookup ?(is_scout = false) (env : lookup_environment)
      (lookup_stack : Ident.t list) (acl0 : annotated_clause)
      (relstack : Relative_stack.t) : symbol M.m =
    let open M in
    let%bind acl1 = pick @@ preds acl0 env.le_cfg in
    let zeromsg msg () =
      lazy_logger `trace (fun () ->
          Printf.sprintf
            "ZERO (%s) for lookup of\n\
            \  %s\n\
            \  with stack %s\n\
            \  at %s\n\
            \  after %s\n"
    ...
    let _ = zeromsg in
    let%bind () = pause () in
    let recurse ?(is_scout = false) (lookup_stack' : Ident.t list)
        (acl0' : annotated_clause) (relstack' : Relative_stack.t) : symbol M.m =
      lazy_logger `trace (fun () ->
          Printf.sprintf
            "From lookup of\n\
            \  %s\n\
      ...
      [
        ( `Discovery,
          ...
          let%orzero (Unannotated_clause
                       (Abs_clause (Abs_var x, Abs_value_body _))) =
            acl1
          in
          ...
          if equal_ident lookup_var env.le_first_var then (
            (* Then we've found the start of the program!  Build an
               appropriate concrete stack. *)
            lazy_logger `trace (fun () ->
                "This lookup complete; top of program reached.");
            let%bind _ =
              record_constraint
              @@ Constraint.Constraint_stack (Relative_stack.stackize relstack)
            in
            return lookup_symbol)
          else (
            lazy_logger `trace (fun () ->
                "This lookup complete; resuming to top of program.");
            let%bind _ = recurse [ env.le_first_var ] acl1 relstack in
            return lookup_symbol) );
        ( `Discard,
        ...
        ( `Skip,
          ...
          let%orzero (Unannotated_clause (Abs_clause (Abs_var x'', _))) =
            acl1
          in
        ...
        ( `Input,
          ...
          let%orzero (Unannotated_clause
                       (Abs_clause (Abs_var x, Abs_input_body))) =
            acl1
          in
          [%guard equal_ident x lookup_var];
          (* Induce the resulting formula *)
        ...
        ( `Alias,
          ...
          | Some (Constraint.Int n) ->
              let%bind () =
                record_constraint
                @@ Constraint.Constraint_value (lookup_symbol, Constraint.Int n)
              in
              return lookup_symbol
          | Some (Constraint.Bool b) ->
              let%bind () =
                record_constraint
                @@ Constraint.Constraint_value (lookup_symbol, Constraint.Bool b)
              in
              return lookup_symbol
        ...
        ( `Binop,
          ...
          if not @@ List.is_empty lookup_stack' then
            raise
            @@ Jhupllib.Utils.Not_yet_implemented
        ...
        ( `FunEnter,
          ...
          let%orzero (Abs_clause (Abs_var xr, Abs_appl_body (Abs_var xf, _))) =
            c
          in
          ...
          let%bind () =
            ...
            @@ Constraint.Constraint_value
                 (function_symbol, Constraint.Function fv)
          in
        ...
        ( `FunEnterNonLocal,
          ...
          (* This must be a binding enter clause which DOES NOT define our
                   lookup variable. *)
          let%orzero (Binding_enter_clause (Abs_var x'', Abs_var x', c)) =
            acl1
          in
        ...
        ( `CondTop,
          ...
          let%orzero (Abs_clause (_, Abs_conditional_body (Abs_var x1, _, _))) =
            c
          in
        ...
        ( `CondBtmTrue,
          ...
          let%orzero (Abs_clause (_, Abs_conditional_body (Abs_var x1, e1, _)))
              =
            c
          in
        ...
        ( `CondBtmFalse,
          ...
          let%orzero (Abs_clause (_, Abs_conditional_body (Abs_var x1, _, e2)))
              =
            c
          in
          ...
          let%bind () =
            record_constraint
            @@ Constraint.Constraint_value
                 (subject_symbol, Constraint.Bool false)
          in
          (* Proceed to look up the value returned by this branch. *)
          recurse (x' :: lookup_stack') acl1 relstack );
        ( `Projection,
          ...
          let%orzero (Unannotated_clause
                       (Abs_clause
                         (Abs_var x, Abs_projection_body (Abs_var x', lbl)))) =
            acl1
          in
          [%guard equal_ident x lookup_var];
    ...
    in
    let rules =
      List.map (fun name -> List.assoc name rule_computations) rule_names
    in

    let%bind m = pick @@ List.enum rules in
    m
  ...
  let start (cfg : ddpa_graph) (e : expr) (program_point : ident) : evaluation =
    ...
    let m : unit m =
      ...
      let%bind _ = lookup env [ initial_lookup_var ] acl Relative_stack.empty in
      return ()
    in
  ...
  let step (x : evaluation) : evaluation_result list * evaluation option =
    ...
    let results' =
      results
      |> Enum.filter_map (fun evaluation_result ->
...
let start ?(exploration_policy = Explore_breadth_first) (cfg : ddpa_graph)
    (e : expr) (x : ident) : evaluation =
