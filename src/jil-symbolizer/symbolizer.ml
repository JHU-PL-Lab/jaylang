...
module Make_exp_helper (Sym_val : Sym_val_sig.S) (Sym_key : Sym_key.S) = struct
  open Sym_val
  include Symbolizer_helper

  let top_stack = Sym_val.var_s "Topstack"

  let picked (key : Lookup_key.t) =
    "P_" ^ Lookup_key.to_string key |> Sym_val.mk_bool_s

  let picked_string (s : string) = "P_" ^ s |> Sym_val.mk_bool_s

  (* Solver primitives *)
  let ( @=> ) = Sym_val.( @=> )
  let true_ = box_bool true
  let false_ = box_bool false
  let bool_ = Sym_val.bool_
  let and_ = Sym_val.and_
  let z_of_fid (Id.Ident fid) = Sym_val.fun_ fid
...
module Make_sym_exp (Sym_val : Sym_val_sig.S) (Sym_key : Sym_key.S) = struct
  ...
  let stack_in_main r_stk =
    Sym_val.eq top_stack
     ...
     |> Sexp.to_string_mach |> Sym_val.fun_)
  ...
  let is_picked model key =
    Option.value_map model ~default:false ~f:(fun model ->
        Option.value (Sym_val.get_bool model (picked key)) ~default:true)
  ...
  let cond_top key key_x key_c beta =
    imply key [ K2 (key, key_x); Z (key_c, Sym_val.bool_ beta) ]
  ...
  (* Fun Enter Local *)
  let fun_enter_local (key_para : Lookup_key.t) p_stk p_fid =
    let cs =
      List.map p_stk ~f:(fun (key_f, key_arg) ->
          [ K2 (key_para, key_arg); Z (key_f, z_of_fid p_fid) ])
    in
    choices key_para cs
  ...
  let pattern x x' key_rv rv pat =
    ...
    let value_pattern =
      ...
      then
        ...
        | Some v -> Sym_val.inject_bool (and2 type_pattern (box_bool v))
        | None -> Sym_val.inject_bool type_pattern
      else Sym_val.inject_bool type_pattern
    in
    imply x
      ([
         Z (x, value_pattern);
...
module Make (Sym_val : Sym_val_sig.S) (Sym_key : Sym_key.S) = struct
  include Sym_val
  include Sym_key
