...
and pp_binop (formatter : Format.formatter) (expr : expr) (with_tag : bool) :
    unit =
  let pp_expr_desc =
    if with_tag then pp_expr_desc_with_tag else pp_expr_desc_without_tag
  in
...
and pp_expr (formatter : Format.formatter) (expr : expr) : unit =
  ...
  (* Operations *)
  | Appl _ -> pp_binop formatter expr false
  | Let (ident, e1, e2) ->
  ...
  | LetRecFun (funsig_lst, e) ->
      Format.fprintf formatter "let rec@ %a@ in@ @[%a@]" pp_funsig_list
        funsig_lst pp_expr_desc e
  | LetFun (funsig, e) ->
      Format.fprintf formatter "let@ %a@ in@ @[%a@]" pp_funsig funsig
        pp_expr_desc e
  | If (pred, e1, e2) ->
...
and pp_expr_with_tag (formatter : Format.formatter) (expr : expr) : unit =
  ...
  | List e_list ->
      Pp_utils.pp_concat_sep_delim "[" "]" ","
        ...
        formatter (List.enum e_list)
  (* Operations *)
  | Appl _ -> pp_binop formatter expr true
  | Let (ident, e1, e2) ->
      Format.fprintf formatter "let@ %a =@ %a@ in@ @[%a@]" pp_ident ident
        pp_expr_desc e1 pp_expr_desc e2
  | LetRecFun (funsig_lst, e) ->
