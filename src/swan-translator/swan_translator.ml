...
let rec nested_function_value_of_swan_function_value
    (Swan_ast.Function(_,Swan_ast.Swan_var(_,x'),e')) =
  let body = nested_expr_of_swan_expr e' in
  (Nested_ast.Function(next_uid(),Nested_ast.Nested_var(next_uid (),x'),body))
...
and nested_expr_of_swan_expr e =
  ...
  | Swan_ast.Var_expr(_,Swan_ast.Swan_var(_,x)) -> Nested_ast.Var_expr(next_uid (),Nested_ast.Nested_var(next_uid (), x))
  ...
  | Swan_ast.If_expr(_,e',e1,e2) ->
    Nested_ast.Conditional_expr(next_uid (),
      ...
      Nested_ast.Bool_pattern(next_uid (),true),
                                nested_function_value_of_swan_function_value (Swan_ast.Function(next_uid (), Swan_ast.Swan_var(next_uid (),fresh_var ()), e1)),
                                nested_function_value_of_swan_function_value (Swan_ast.Function(next_uid (), Swan_ast.Swan_var(next_uid (),fresh_var ()), e2))
    )
  ...
  | Swan_ast.Indexing_expr(_,e1,e2) ->
    Nested_ast.Indexing_expr(next_uid (),
      nested_expr_of_swan_expr e1,
      nested_expr_of_swan_expr e2)
  | Swan_ast.Let_expr(_,Swan_ast.Swan_var(_,x),e1,e2) ->
    Nested_ast.Let_expr(next_uid (),Nested_ast.Nested_var(next_uid (),x),
                        nested_expr_of_swan_expr e1,
                        nested_expr_of_swan_expr e2)
  ...
  | Swan_ast.Match_expr(_,e, ms) ->
    ...
    let rec desugar_matches ms e =
      ...
      | Swan_ast.Match_pair(_,p,e')::ms' -> Nested_ast.Conditional_expr(next_uid (),
         nested_expr_of_swan_expr e,
         nested_pattern_of_swan_pattern p,
                                                                        (Nested_ast.Function(next_uid (),Nested_ast.Nested_var(next_uid (),fresh_var ()), nested_expr_of_swan_expr e')),
                                                                        (Nested_ast.Function(next_uid (),Nested_ast.Nested_var(next_uid (),fresh_var ()), desugar_matches ms' e)))
