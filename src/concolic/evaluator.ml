...
let eval_exp 
  ...
  let rec eval (expr : Embedded.t) : Value.t m =
    ...
    | EFunction { param ; body } ->
      let%bind env = read_env in
      return @@ VFunClosure (ref (param, { expr = body ; env }, []))
    | EId -> return VId
    ...
    | EProject { record = e_record ; label } -> begin
      ...
      | _ ->
        type_mismatch @@ Error_msg.project_non_record label v
    end
    ...
    | EAppl { func ; arg } -> begin
      ...
      | VFunClosure c1 -> begin
        ...
        |> function
          | Some res -> return res
          | None ->
            let%bind res = local (fun _ -> Env.add param varg body.env) (eval body.expr) in
