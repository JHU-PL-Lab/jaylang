...
let new_translation_context
  ...
  { tc_fresh_name_counter = 0;
    ...
    tc_odefa_instrumentation_mappings = Odefa_instrumentation_maps.empty is_natodefa;
  }
;;

let new_translation_context_from_natodefa
    ?suffix:(suffix=("~"))
    ?contextual_recursion:(contextual_recursion=(true))
    natodefa_inst_map
  : translation_context =
  { tc_fresh_name_counter = 0;
    tc_fresh_suffix_separator = suffix;
    tc_contextual_recursion = contextual_recursion;
    tc_odefa_instrumentation_mappings = Odefa_instrumentation_maps.inherit_from_on_to_odefa_maps natodefa_inst_map;
  }
;;
...
module TranslationMonad : sig
  ...
  let add_var_clause_pair v_key cls_val ctx =
    ...
    ctx.tc_odefa_instrumentation_mappings
      <- Odefa_instrumentation_maps.add_odefa_var_clause_mapping odefa_inst_maps i_key cls_val
  ;;
  ...
  let add_instrument_var_pair v_key v_val ctx =
    ...
    let odefa_inst_maps = ctx.tc_odefa_instrumentation_mappings in
    ctx.tc_odefa_instrumentation_mappings
      <- Odefa_instrumentation_maps.add_odefa_instrument_var odefa_inst_maps i_key (Some i_val);
  ;;
