...
let[@landmark] des_to_many_emb (des : Desugared.pgm) ~(do_wrap : bool) ~(do_type_splay : bool) : Embedded.pgm Preface.Nonempty_list.t =
  let module Names = Translation_tools.Fresh_names.Make () in
  des
...
let[@landmark] some_program_to_emb (prog : some_program) ~(do_wrap : bool) ~(do_type_splay : bool) : Embedded.pgm =
  match prog with
...
let[@landmark] some_program_to_many_emb (prog : some_program) ~(do_wrap : bool) ~(do_type_splay : bool) : Embedded.pgm Preface.Nonempty_list.t =
  match prog with
  | SomeProgram(BluejayLanguage, bjy_prog) ->
