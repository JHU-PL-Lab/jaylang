...
let raw_bluejay_to_jayil ~is_instrumented raw_bluejay =
  let bluejay_ast = raw_bluejay |> Bluejay.Bluejay_ast.new_expr_desc in
  let init_consts = bluejay_edesc_to_consts bluejay_ast in
  bluejay_edesc_to_jayil ~do_wrap:true ~is_instrumented bluejay_ast
    ~consts:init_consts

let raw_bluejay_to_jay raw_bluejay =
  raw_bluejay |> Bluejay.Bluejay_ast.new_expr_desc
  |> bluejay_edesc_to_jay ~do_wrap:true

let raw_jay_to_jayil ~is_instrumented raw_jay =
  let jay_ast = raw_jay |> Jay.Jay_ast.new_expr_desc in
  let consts = jay_edesc_to_consts jay_ast in
  jay_edesc_to_jayil ~is_instrumented ~consts jay_ast
