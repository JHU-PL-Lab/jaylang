...
module Debug = struct
  ...
  let raise_if_with_stack session x stk v =
    ...
    | Session.Concrete.Mode.With_full_target (target_x, target_stk)
      when Ident.equal target_x x ->
  ...
  let debug_clause ~conc_session x v stk =
    ...
    | Session.Concrete.Mode.Debug.Debug_clause clause_cb ->
        clause_cb x stk (Dvalue.value_of_t v)
...
module Fetch = struct
  let fetch_val_with_stk ~(conc_session : Session.Concrete.t) ~stk env
      (Var (x, _)) : Dvalue.t * Concrete_stack.t =
    let res = Ident_map.find x env in
    (* find the variable and stack in the environment *)
    Debug.debug_update_read_node conc_session x stk ;
    res

  let fetch_val ~(conc_session : Session.Concrete.t) ~stk env x : Dvalue.t =
    fst (fetch_val_with_stk ~conc_session ~stk env x)
  (* find variable and stack, then discard stack *)

  let fetch_stk ~(conc_session : Session.Concrete.t) ~stk env x :
      Concrete_stack.t =
    snd (fetch_val_with_stk ~conc_session ~stk env x)
  (* find variable and stack, then discard variable *)

  let fetch_val_to_direct ~(conc_session : Session.Concrete.t) ~stk env vx :
      value =
  ...
  let check_pattern ~(conc_session : Session.Concrete.t) ~stk env vx pattern :
      bool =
...
let rec eval_exp ~(conc_session : Session.Concrete.t)
    ~(* Note: is mutable *)
    (symb_session : Session.Symbolic.t) (stk : Concrete_stack.t)
    (env : Dvalue.denv) (e : expr) : Dvalue.denv * Dvalue.t * Session.Symbolic.t
    =
  ILog.app (fun m -> m "@[-> %a@]\n" Concrete_stack.pp stk) ;
  ...
  | With_full_target (_, target_stk) ->
      ...
      Hashtbl.change conc_session.rstk_picked r_stk ~f:(function
        ...
        | Some false ->
            raise (Run_into_wrong_stack (Jayil.Ast_tools.first_id e, stk))
        | None -> None)
  | _ -> ()) ;
  let (Expr clauses) = e in
  let (denv, conc_session), vs =
    List.fold_map clauses ~init:(env, symb_session) ~f:(fun (env, pt) clause ->
        let denv, v, pt =
          eval_clause ~conc_session ~symb_session:pt stk env clause
        in
        ((denv, pt), v))
  in
  (denv, List.last_exn vs, conc_session)

and eval_clause ~(conc_session : Session.Concrete.t)
    ~(symb_session : Session.Symbolic.t) (stk : Concrete_stack.t)
    (env : Dvalue.denv) (clause : clause) :
  ...
  | Some max_step ->
      ...
      then
        raise
          (Reach_max_step (x, stk, Session.Symbolic.reach_max_step symb_session))
      else ()) ;
  ...
  let (v, symb_session) : Dvalue.t * Session.Symbolic.t =
    ...
    | Var_body vx ->
        ...
        let (Var (y, _)) = vx in
        let ret_val, ret_stk =
          Fetch.fetch_val_with_stk ~conc_session ~stk env vx
        in
    ...
    | Conditional_body (cx, e1, e2) ->
        ...
        let (Var (y, _)) = cx in
        let cond_val, condition_stk =
          Fetch.fetch_val_with_stk ~conc_session ~stk env cx
        in
        let cond_bool =
          match cond_val with
          | Direct (Value_bool b) -> b
        ...
        let this_branch =
          Branch.Runtime.
            {
              branch_key = get_key x_key;
              condition_key = get_key condition_key;
              direction = Branch.Direction.of_bool cond_bool;
            }
        in

        (* enter/hit branch *)
        let symb_session =
          Session.Symbolic.hit_branch symb_session this_branch
        in
        ...
        let ret_env, ret_val, symb_session =
          eval_exp ~conc_session ~symb_session stk' env e
        in
        let (Var (ret_id, _) as last_v) = Jayil.Ast_tools.retv e in
        (* last defined value in the branch *)
        let _, ret_stk =
          Fetch.fetch_val_with_stk ~conc_session ~stk:stk' ret_env last_v
        in

        (* say the ret_key is equal to x now, then clear out branch *)
        let ret_key = make_key ret_id ret_stk in
        let symb_session =
          Session.Symbolic.add_alias symb_session x_key ret_key
        in
    ...
    | Appl_body (vf, (Var (x_arg, _) as varg)) -> (
        (* x = f y ; *)
        ...
        | FunClosure (fid, Function_value (Var (param, _), body), fenv) ->
            ...
            let arg, arg_stk =
              Fetch.fetch_val_with_stk ~conc_session ~stk env varg
            in
            let stk' = Concrete_stack.push (x, fid) stk in
            let env' = Ident_map.add param (arg, stk') fenv in
            Session.Concrete.add_alias (param, stk) (x_arg, arg_stk)
              conc_session ;
            ...
            let symb_session =
              Session.Symbolic.add_alias symb_session key_param key_arg
            in

            (* returned value of function *)
            let ret_env, ret_val, symb_session =
              eval_exp ~conc_session ~symb_session stk' env' body
            in
            let (Var (ret_id, _) as last_v) = Jayil.Ast_tools.retv body in
            let ret_stk =
              Fetch.fetch_stk ~conc_session ~stk:stk' ret_env last_v
            in
            Session.Concrete.add_alias (x, stk) (ret_id, ret_stk) conc_session ;
    ...
    | Match_body (vy, p) ->
        (* x = y ~ <pattern> ; *)
        let match_res =
          Value_bool (Fetch.check_pattern ~conc_session ~stk env vy p)
        in
    ...
    | Projection_body (v, label) -> (
        ...
        | RecordClosure (Record_value r, denv) ->
            ...
            let retv, stk' =
              Fetch.fetch_val_with_stk ~conc_session ~stk denv proj_v
            in
    ...
    | Binary_operation_body (vy, op, vz) ->
        ...
        ( retv,
          Session.Symbolic.add_binop symb_session x_key op y_key z_key
          (* just adding keys, not any runtime values, so does not need to be implied by results of earlier branches *)
        )
    | Abort_body -> (
        ...
        | Plain ->
            raise
            @@ Found_abort (ab_v, Session.Symbolic.found_abort symb_session)
        (* no need to "exit" or anything. Just say interpretation stops. *)
    ...
    | Assert_body cx | Assume_body cx ->
        ...
        then
          raise
          @@ Found_failed_assume (Session.Symbolic.fail_assume symb_session)
          (* fail the assume that was just found *)
        else
          let retv = Direct (Value_bool v) in
          Session.Concrete.add_val_def_mapping (x, stk) (cbody, retv)
            conc_session ;
          ( retv,
            symb_session
            (*Session.Symbolic.add_key_eq_val symb_session x_key (Value_bool v) *)
          )
  in
  Debug.debug_clause ~conc_session x v stk ;
  (Ident_map.add x (v, stk) env, v, symb_session)

let eval_exp_default ~(conc_session : Session.Concrete.t)
    ~(symb_session : Session.Symbolic.t) (e : expr) :
    Dvalue.denv * Dvalue.t * Session.Symbolic.t =
  eval_exp ~conc_session ~symb_session Concrete_stack.empty (* empty stack *)
...
let try_eval_exp_default ~(conc_session : Session.Concrete.t)
    ~(symb_session : Session.Symbolic.t) (e : expr) : Session.Symbolic.t =
  try
  ...
  | Reach_max_step (_, _, symb_session) ->
      ...
      symb_session
  | Found_failed_assume symb_session | Found_failed_assert symb_session ->
...
*)
let rec loop (e : expr) (prev_session : Session.t) :
    (Branch_info.t * bool) Lwt.t =
  let open Lwt.Infix in
  let%lwt () = Lwt.pause () in
  Session.next prev_session |> function
  | `Done (branch_info, has_pruned) ->
      Log.Export.CLog.app (fun m ->
          m
            "\n\
             ------------------------------\n\
             Finishing concolic evaluation...\n\n") ;
      Log.Export.CLog.app (fun m ->
          m "Ran %d interpretations.\n" (Session.run_num prev_session)) ;
  ...
  | `Next (session, symb_session, conc_session) ->
      ...
      Log.Export.CLog.info (fun m ->
          m "%s" (Branch_info.to_string @@ Session.branch_info session)) ;
      ...
      let resulting_symbolic =
        try_eval_exp_default ~conc_session ~symb_session e
      in
      let t1 = Caml_unix.gettimeofday () in
      Log.Export.CLog.app (fun m ->
          m "Interpretation finished in %fs.\n\n" (t1 -. t0)) ;
      loop e @@ Session.accum_symbolic session resulting_symbolic

(* TODO: maybe move some of this to driver *)
let lwt_eval :
    (Jayil.Ast.expr -> (Branch_info.t * bool) Lwt.t) Concolic_options.Fun.t =
...
let[@landmark] eval : (Jayil.Ast.expr -> Branch_info.t) Concolic_options.Fun.t =
  let f (r : Concolic_options.t) (e : Jayil.Ast.expr) =
    try
      let t0 = Caml_unix.gettimeofday () in
      let res =
        Tuple2.get1 @@ Lwt_main.run @@ Concolic_options.Fun.appl lwt_eval r e
      in
      Log.Export.CLog.app (fun m ->
    ...
    with Lwt_unix.Timeout ->
      Log.Export.CLog.app (fun m ->
          m "Quit due to total run timeout in %0.3f seconds.\n"
...
module Test_result = struct
  type t =
    | Found_abort of Branch.t * Jil_input.t list
      (* Found an abort at this branch using these inputs *)
    | Exhausted (* Ran all possible tree paths, and no paths were too deep *)
    | Exhausted_pruned_tree
      (* Ran all possible tree paths up to the given max step *)
...
let[@landmark] test : (Jayil.Ast.expr -> Test_result.t) Concolic_options.Fun.t =
  let f (r : Concolic_options.t) (e : Jayil.Ast.expr) =
    try
      let t0 = Caml_unix.gettimeofday () in
      let res, has_pruned =
        Lwt_main.run @@ Concolic_options.Fun.appl lwt_eval r e
      in
      Log.Export.CLog.app (fun m ->
          m "\nFinished concolic evaluation in %fs.\n"
            (Caml_unix.gettimeofday () -. t0)) ;
      Branch_info.find res ~f:(fun _ -> function
        | Branch_info.Status.Found_abort _ -> true | _ -> false)
      |> function
