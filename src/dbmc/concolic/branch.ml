...
module Direction = struct
  type t = True_direction | False_direction
...
module T = struct
  type t = { branch_ident : Ast.Ident_new.t; direction : Direction.t }
...
module Runtime = struct
  ...
  let print_target_option (x : t option) : unit =
    let target_branch_str =
      match x with None -> "None" | Some target -> to_string target
    in
    Format.printf "\nTarget branch: %s\n" target_branch_str

  let pick_abort (x : t) : Z3.Expr.expr = T.pick_abort @@ to_ast_branch x
  let pick_max_step (x : t) : Z3.Expr.expr = T.pick_max_step @@ to_ast_branch x
end
