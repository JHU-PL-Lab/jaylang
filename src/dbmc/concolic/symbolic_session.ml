...
module Node_stack = struct
  type t = Last of Root.t | Cons of Child.t * t
  ...
  let map_hd (stack : t) ~(f : Node.t -> Node.t) : t =
    ...
    | Cons (hd, tl) -> Cons (Child.map_node hd ~f, tl)
  (* might be a good choice to throw exception if node doesn't exist *)
  ...
  let to_tree (stack : t) : Root.t =
    let rec to_tree acc = function
      ...
      | Cons (({ status = Hit node; _ } as child), tl) ->
          (* branch was hit, and no failed assert or assume *)
          let acc =
            Children.set_node Children.empty child.branch
              { node with children = acc }
          in
          to_tree acc tl
      | Cons (({ status = Failed_assume; _ } as child), tl) ->
          (* branch was hit, and failed assert of assume *)
          assert (Children.is_empty acc) ;
          let acc = Children.set_child Children.empty child in
          (* has no node in which to set children *)
          to_tree acc tl
      | _ -> failwith "logically impossible"
      (* impossible if the code does as I expect it *)
    in
  ...
  let get_targets (stack : t) (tree : Root.t) : Target.t list =
    ...
    (* this path leads to all possible targets *)
    let rec step (cur_targets : Target.t list) (cur_node : Node.t)
        (remaining_path : Path.t) (path : Path.t) : Target.t list =
      match remaining_path with
      | last_branch :: [] ->
          (* maybe last node hit should be target again in case it failed assume or assert *)
          push_target
            (push_target cur_targets cur_node last_branch path)
            (* push this direction *)
            cur_node
            (Branch.Runtime.other_direction last_branch)
            (* push other direction *)
            path
      | branch :: tl ->
          step
            (push_target cur_targets cur_node
               (Branch.Runtime.other_direction branch)
               path)
            (* push other direction as possible target *)
            (Child.to_node_exn @@ Node.get_child_exn cur_node branch)
            (* step down path *)
            tl (* continue down remainder of path *)
            (path @ [ branch ])
          (* complexity of this is bad, but depth is so shallow that is not slow in practice *)
      | [] -> cur_targets
    and push_target (cur_targets : Target.t list) (cur_node : Node.t)
        (branch : Branch.Runtime.t) (path : Path.t) : Target.t list =
      if Node.is_valid_target_child cur_node branch
      then
        Target.create (Node.get_child_exn cur_node branch) path :: cur_targets
      else cur_targets
    in
  ...
  let merge_with_tree (allowed_tree_depth : int) (stack : t) (tree : Root.t) :
      Root.t * Target.t list =
    let merged = Root.merge tree @@ to_tree stack in
...
module Depth_logic = struct
  type t = { cur_depth : int; max_depth : int; is_below_max : bool }
  (** [t] helps track if we've reached the max tree depth and thus should stop
      creating formulas *)
  ...
  let incr (x : t) : t =
    {
      x with
      cur_depth = x.cur_depth + 1;
      is_below_max = x.cur_depth < x.max_depth;
    }
end
...
module Basic = struct
  ...
  let with_options : (t -> t) Concolic_options.Fun.t =
    Concolic_options.Fun.make @@ fun (r : Concolic_options.t) (x : t) ->
    { x with depth = { x.depth with max_depth = r.max_tree_depth } }

  let found_abort (s : t) : t =
    {
      s with
      branch_info =
        Branch_info.set_branch_status ~new_status:(Found_abort s.inputs)
          s.branch_info
  ...
  let found_assume (s : t) (cx : Lazy_key.t) : t =
    ...
    | Last _ ->
        s
        (* `assume` found in global scope. We assume this is a test case that can't happen in real world translations to JIL *)
    | Cons (hd, tl) ->
        let new_hd =
  ...
  let failed_assume (s : t) : t =
    ...
    | Cons (hd, tl) ->
        let constraints =
          Formula_set.add_multi hd.constraints @@ Child.to_formulas hd
        in
        let new_hd =
          Child.
            {
              status =
                Status.Failed_assume
                (* forget all formulas so that it is a possible target in future runs *);
              constraints =
                Formula_set.add_multi hd.constraints
                @@ Child.to_formulas hd (* constrain to passing assume/assert *);
              branch = hd.branch;
            }
        in
        { s with stack = Cons (new_hd, tl) }
...
module At_max_depth = struct
  type t = { last_branch : Branch.t; base : Basic.t }

  let of_basic (s : Basic.t) : t =
    { last_branch = Node_stack.hd_branch_exn s.stack; base = s }

  let with_options : (t -> t) Concolic_options.Fun.t =
    Concolic_options.Fun.make @@ fun (r : Concolic_options.t) (x : t) ->
    { x with base = Concolic_options.Fun.appl Basic.with_options r x.base }
      ...
      base =
        {
          ...
          branch_info =
            Branch_info.set_branch_status
              ~new_status:(Found_abort a.base.inputs) a.base.branch_info
              a.last_branch;
        };
    }
end
...
module Finished = struct
  ...
  let with_options : (t -> t) Concolic_options.Fun.t =
    Concolic_options.Fun.make @@ fun (r : Concolic_options.t) (x : t) ->
    { x with prev = Concolic_options.Fun.appl Basic.with_options r x.prev }

  let of_basic (s : Basic.t) (root : Root.t) : t =
    (* logically sound to have hit target if formulas are consistent with JIL program *)
    let tree, targets =
      Node_stack.merge_with_tree s.depth.max_depth s.stack root
    in
    assert (
      Option.is_none s.target || Target.is_hit (Option.value_exn s.target) tree) ;
    (* check that target was hit in new tree *)
...
let with_options : (t -> t) Concolic_options.Fun.t =
  ...
  | At_max_depth s ->
      At_max_depth (Concolic_options.Fun.appl At_max_depth.with_options r s)
...
let add_lazy_formula (x : t) (lazy_expr : unit -> Z3.Expr.expr) : t =
  match x with
  | Basic s ->
      Basic { s with stack = Node_stack.add_formula s.stack @@ lazy_expr () }
  | At_max_depth _ -> x
...
let hit_branch (x : t) (branch : Branch.Runtime.t) : t =
      ...
      Basic
        {
          s with
          branch_info =
            Branch_info.set_branch_status ~new_status:Hit s.branch_info
  ...
  | At_max_depth a ->
      At_max_depth { a with last_branch = Branch.Runtime.to_ast_branch branch }
...
let reach_max_step (x : t) : t =
  ...
  | At_max_depth a ->
      At_max_depth { a with base = { a.base with reach_max_step = true } }
...
let add_input (x : t) (key : Lazy_key.t) (v : Dvalue.t) : t =
  let key = key () in
  (* assume it's not that expensive to compute the key on inputs *)
  let n =
    match v with
...
let add_match (x : t) (k : Lazy_key.t) (m : Lazy_key.t)
  ...
  Riddler.mk_eq (Riddler.project_bool k_expr) (Riddler.if_pattern (m ()) pat)
...
let[@landmarks] finish (x : t) (tree : Root.t) : t =
  ...
  | At_max_depth a ->
      Finished { (Finished.of_basic a.base tree) with hit_max_depth = true }
  | Finished r -> Finished { r with tree }
(* allow finishing twice by reseting the tree *)

let make (root : Root.t) (target : Target.t) : t =
  Basic
    { Basic.empty with stack = Node_stack.of_root root; target = Some target }

let root_exn (x : t) : Root.t =
  match x with Finished { tree; _ } -> tree | _ -> failwith "no root"

let targets_exn (x : t) : Target.t list =
  match x with Finished { targets; _ } -> targets | _ -> failwith "no targets"

let branch_info (x : t) : Branch_info.t =
  match x with
  | Basic { branch_info; _ }
  | At_max_depth { base = { branch_info; _ }; _ }
  | Finished { prev = { branch_info; _ }; _ } ->
      branch_info
...
let is_reach_max_step (x : t) : bool =
  ...
  | Finished { prev = { reach_max_step; _ }; _ } ->
      reach_max_step
