...
module Concrete = struct
  ...
  let create (input_feeder : Input_feeder.t) (global_max_step : int) : t =
    { (create_default ()) with input_feeder; max_step = Some global_max_step }

  (* Say that x1 is an alias for x2. x1 is defined *after* x2 and points to x2. *)
  let add_alias (x1 : Id_with_stack.t) (x2 : Id_with_stack.t)
      ({ alias_graph; _ } : t) : unit =
    G.add_edge alias_graph x1 x2

  (* Say that x is the variable for the clause body that evaluates to dvalue *)
  let add_val_def_mapping (x : Id_with_stack.t) (vdef : clause_body * Dvalue.t)
      ({ val_def_map; _ } : t) : unit =
...
type t = {
  ...
  quit : bool;
  has_pruned : bool;
      (* true iff some evaluation hit more nodes than are allowed to be kept *)
...
let accum_symbolic (x : t) (sym : Symbolic.t) : t =
  let sym = Symbolic.finish sym x.tree in
  {
    ...
    target_queue =
      Target_queue.push_list x.target_queue @@ Symbolic.targets_exn sym;
    quit =
      x.quit
      || x.options.quit_on_abort
         && Branch_info.contains (Symbolic.branch_info sym) (Found_abort []);
    last_sym = Some sym;
  }

let[@landmarks] check_solver solver = Z3.Solver.check solver []
let[@landmarks] make_solver () = Z3.Solver.mk_solver Solver_helper.ctx None
...
let[@landmarks] check_solver' formulas =
  let new_solver = Z3.Solver.mk_solver Solver_helper.ctx None in
...
let[@landmarks] next (x : t) :
    [ `Done of Branch_info.t * bool | `Next of t * Symbolic.t * Concrete.t ] =
  let pop_kind =
    match x.last_sym with
    | Some s when Symbolic.is_reach_max_step s ->
        `BFS (* only does BFS when last symbolic run reached max step *)
    | _ -> `Random
  in
  let rec next (x : t) :
      [ `Done of Branch_info.t * bool | `Next of t * Symbolic.t * Concrete.t ] =
  ...
  and solve_for_target (x : t) (target : Target.t) =
    ...
    | Z3.Solver.UNSATISFIABLE ->
        ...
        Log.Export.CLog.info (fun m ->
            m "FOUND UNSATISFIABLE in %fs\n" (t1 -. t0)) ;
        next
          {
            x with
            tree =
              Root.set_status x.tree target.child Status.Unsatisfiable
                target.path;
          }
    | Z3.Solver.UNKNOWN ->
        Log.Export.CLog.info (fun m ->
            m "FOUND UNKNOWN DUE TO SOLVER TIMEOUT\n") ;
        next
          {
            x with
            tree =
              Root.set_status x.tree target.child Status.Unknown target.path;
          }
    | Z3.Solver.SATISFIABLE ->
        Log.Export.CLog.app (fun m ->
            m "FOUND SOLUTION FOR BRANCH: %s\n"
              (Branch.to_string
              @@ Branch.Runtime.to_ast_branch target.child.branch)) ;
        `Next
          ( { x with run_num = x.run_num + 1 },
            apply_options_symbolic x @@ Symbolic.make x.tree target,
            Z3.Solver.get_model new_solver
            |> Core.Option.value_exn |> Concolic_feeder.from_model
            |> fun feeder -> Concrete.create feeder x.options.global_max_step )
  and done_ (x : t) =
    Log.Export.CLog.info (fun m ->
        m "Done. Tree size is %d\n" (Root.size x.tree)) ;
    `Done (x.branch_info, x.has_pruned)
  in

  next x

let branch_info ({ branch_info; _ } : t) : Branch_info.t = branch_info
let run_num ({ run_num; _ } : t) : int = run_num
