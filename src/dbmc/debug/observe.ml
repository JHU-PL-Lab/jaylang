...
let process_rstk_stat_map (config : Global_config.t) (state : Global_state.t) =
  let raw_plist = state.stat.rstk_stat_map |> Hashtbl.to_alist in
  let srd_list =
    raw_plist
...
let dump_block_stat (config : Global_config.t) (state : Global_state.t) =
  ...
  then
    let raw_plist = state.stat.block_stat_map |> Hashtbl.to_alist in
    LS2Log.debug (fun m ->
        m "%d@,%a" state.search.tree_size
...
let update_block_visits (config : Global_config.t) (state : Global_state.t)
  ...
  Hashtbl.update state.stat.rstk_stat_map key.r_stk ~f:(function
    | None ->
        {
          block_id = key.block.id;
          visits = 1;
          smt_checks = (if is_checked then 1 else 0);
          smt_size = Riddler.get_assertion_count state.solve.solver;
          smt_time;
        }
    | Some d ->
        {
          ...
          smt_size =
            max d.smt_size (Riddler.get_assertion_count state.solve.solver);
          smt_time = d.smt_time +. smt_time;
        }) ;
  let block = key.block.id in
  Hashtbl.update state.stat.block_stat_map key.block ~f:(function
    | None -> { visits = 1 }
    | Some d -> { visits = d.visits + 1 }) ;
  ()

let get_block_visits (state : Global_state.t) (key : Lookup_key.t) =
  Hashtbl.find_and_call state.stat.block_stat_map key.block
...
let get_dbmc_unroll (state : Global_state.t) =
  match state.job.unroll with S_dbmc unroll -> unroll | _ -> failwith "unroll"
...
let dump_check_info filename (state : Global_state.t) =
  List.iter state.stat.check_infos ~f:(fun entry ->
      PLog.debug (fun m ->
