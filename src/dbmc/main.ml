...
let handle_found_verbose (config : Global_config.t) (state : Global_state.t)
  ...
  (* check expected inputs *)
  check_expected_input ~config ~state ;
  ([ inputs_from_interpreter ], true, Some (model, c_stk))
...
let[@landmark] main_with_state_lwt_verbose ~(config : Global_config.t)
    ~(state : Global_state.t) =
  let job_queue =
    let open Scheduler in
    let cmp t1 t2 =
  ...
  let post_check_dbmc () =
    match Riddler.check state config with
    | Some { model; c_stk } -> handle_found_verbose config state model c_stk
    | None ->
        SLog.info (fun m -> m "UNSAT") ;
        if config.debug_model
        then
  ...
  with
  | Riddler.Found_solution { model; c_stk } ->
      Lwt.return (handle_found_verbose config state model c_stk)
  | Lwt_unix.Timeout ->
