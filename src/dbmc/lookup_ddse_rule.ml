...
module Make (S : S) = struct
  ...
  let rule_nonmain v (key : Lookup_key.t) this_node block run_task =
    ...
    Lwt.async (fun () ->
        U.by_map S.state.unroll key key_first (fun _ -> Lookup_result.ok key.x))
  ...
  let discard p key this_node block run_task =
    ...
    Lwt.async (fun () -> U.by_id S.state.unroll key key_drop_x)
  ...
  let alias p (key : Lookup_key.t) this_node block run_task =
    ...
    Lwt.async (fun () -> U.by_id S.state.unroll key key_rx)
  ...
  let record_start p (key : Lookup_key.t) this_node block run_task =
    ...
    Lwt.async (fun () -> U.by_id S.state.unroll key key_r_l)
  ...
  let record_end p (key : Lookup_key.t) this_node block run_task =
    ...
    | Some (Var (vid, _)) ->
        ...
        run_task key' block ;
        Lwt.async (fun () -> U.by_id S.state.unroll key key')
    | None ->
  ...
  let cond_top (cb : Cond_top_rule.t) (key : Lookup_key.t)
    ...
    let cb key (rc : Lookup_result.t) =
      ...
      Lwt.return_unit
    in
    Lwt.async (fun () -> U.by_bind S.state.unroll key key_x2 cb)
  ...
  let cond_btm p (this_key : Lookup_key.t) (this_node : Node.ref_t) block
    ...
    in
    Lwt.async (fun () -> U.by_bind S.state.unroll this_key key_cond_var cb)
  ...
  let fun_enter (fb : Tracelet.fun_block) is_local this_key this_node run_task =
    ...
    Lwt.async (fun () -> U.by_join S.state.unroll this_key lookups)
  ...
  let fun_exit p this_key this_node block run_task =
    ...
    Lwt.async (fun () -> U.by_bind S.state.unroll this_key key_fun cb)
