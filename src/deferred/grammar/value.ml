...
module Concolic_value = struct
  ...
  (*
    All of this equality stuff is copied from concolic values 
    in the eager semantics. It might be a good idea to define
    the eager values in terms of deferred values because it
    is just subtyping.
  *)
  module Equality = struct
    module M = struct
      include Preface.Make.Writer.Over_monad
        (Preface.Option.Monad) 
        (Utils.List_monoid.Make (struct type t = (bool, Interp_common.Timestamp.t) Smt.Formula.t end))
    ...
    let rec equal (a : t) (b : t) : bool M.t =
      let open M in
      ...
      | VTable r1, VTable r2 -> 
        ...
        return b
      (* Symbols *)
      | VSymbol t1, VSymbol t2 ->
        (* TODO: we need to force the evaluation of these symbols *)
        if Interp_common.Timestamp.equal t1 t2
        then return true
        else never_equal (* This is incomplete, and we can fix it once we have a way to evaluate the symbols *)
...
let rec subst (v : t) ~(f : Interp_common.Timestamp.t -> whnf) : Without_symbols.t =
  ...
  (* Unhandled *)
  | VTable { alist } -> VTable { alist = List.map alist ~f:(fun (dom, cod) -> subst ~f dom, subst ~f cod) }