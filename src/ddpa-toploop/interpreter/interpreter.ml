...
let lookup env x =
  if Environment.mem env x
  then Environment.find env x
  else
    raise
      (Evaluation_failure
...
let repl_fn_for clauses freshening_stack extra_bound =
  let bound_variables =
    bound_vars_of_expr clauses |> Var_set.union extra_bound
  in
  let repl_fn (Var (i, _) as x) =
    if Var_set.mem x bound_variables then Var (i, Some freshening_stack) else x
  in
  repl_fn
...
let fun_wire (Function_value (param_x, body_expr)) arg_x call_site_x =
  ...
  let repl_fn =
    repl_fn_for body_expr freshening_stack @@ Var_set.singleton param_x
  in
...
let matches env x p =
  ...
  match (v, p) with
  | _, Any_pattern -> true
  | Value_record (Record_value els), Rec_pattern els' ->
      els' |> Ident_set.enum
      |> Enum.for_all (fun i ->
             try Ident_map.mem i els with Not_found -> false)
  | Value_function (Function_value _), Fun_pattern | Value_int _, Int_pattern ->
      true
  | Value_bool _actual_boolean, Bool_pattern -> true
  | _ -> false

let rec evaluate ?(input_source = stdin_input_source)
    ?(clause_callback = fun (_ : clause) -> ()) env lastvar cls =
  ...
  | (Clause (x, b) as c) :: t -> (
      let recurse = evaluate ~input_source ~clause_callback env (Some x) in
      clause_callback c ;
      match b with
      | Value_body v ->
      ...
      | Appl_body (x', x'') -> (
          ...
          | Value_function f -> recurse @@ fun_wire f x'' x @ t
          | r ->
              raise
                (Evaluation_failure
                   (Printf.sprintf
      ...
      | Conditional_body (x', e1, e2) ->
          ...
          let e_target =
            ...
            | _ ->
                raise
                  (Evaluation_failure
                     (Printf.sprintf "cannot condition on non-boolean value %s"
                        (show_value v)))
          in
      ...
      | Binary_operation_body (x1, op, x2) ->
          ...
          let result =
            ...
            | Value_int n1, Binary_operator_divide, Value_int n2 ->
                if n2 <> 0
                then Value_int (n1 / n2)
                else
                  raise @@ Evaluation_failure ("Divide by zero at " ^ show_var x)
            | Value_int n1, Binary_operator_modulus, Value_int n2 ->
                if n2 <> 0
                then Value_int (n1 mod n2)
                else
                  raise
                  @@ Evaluation_failure ("Modulus by zero at " ^ show_var x)
            | Value_int n1, Binary_operator_less_than, Value_int n2 ->
                Value_bool (n1 < n2)
            | Value_int n1, Binary_operator_less_than_or_equal_to, Value_int n2
              ->
                Value_bool (n1 <= n2)
            ...
            | v1, op, v2 ->
                ...
                @@ Evaluation_failure
                     (Printf.sprintf
                        ...
                        (show_value v1) (show_binary_operator op)
                        (show_value v2))
          in
          Environment.add env x result ;
          recurse t
...
let eval ?(input_source = stdin_input_source)
    ?(clause_callback = fun (_ : clause) -> ()) e =
  ...
  evaluate ~input_source ~clause_callback env None cls
