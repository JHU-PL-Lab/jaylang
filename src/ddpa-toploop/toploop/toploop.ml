...
let stdout_variable_analysis_callback var_name site_name_opt context_opt values
    =
  print_string "\nLookup of variable " ;
  ...
  print_string "    " ;
  print_string @@ Abs_value_set.show values ;
  print_endline "" ;
  flush stdout
...
let stdout_size_report_callback
    ( ddpa_number_of_active_nodes,
      ...
      pds_number_of_edges ) =
  Printf.printf
    "DDPA number of active nodes (excluding enter and exit nodes that can be \
     inferred): %n.\n\
     DDPA number of active non immediate nodes (excluding enter and exit nodes \
     that can be inferred): %n.\n\
     DDPA number of edges: %n.\n\
     PDS number of nodes: %n.\n\
     PDS number of edges: %n.\n"
    ddpa_number_of_active_nodes ddpa_number_of_active_non_immediate_nodes
    ddpa_number_of_edges pds_number_of_nodes pds_number_of_edges ;
  flush stdout

let stdout_source_statistics_callback stats =
  let {
    ss_num_program_points = num_program_points;
    ...
    ss_num_non_local_variable_references = num_non_local_variable_references;
  ...
  Printf.printf
    ...
    num_program_points num_function_definitions num_function_calls
    num_variable_references num_non_local_variable_references max_lexical_depth ;
  Int_map.iter
    (fun depth count ->
      Printf.printf
...
let do_analysis_steps callbacks conf e =
  ...
  if conf.topconf_disable_inconsistency_check
     && conf.topconf_analyze_vars == Analyze_no_variables
    ...
    | Some context_stack ->
        ...
        *)
        ...
        let ddpa_logging_config =
          {
            ddpa_pdr_logging_level;
            ddpa_cfg_logging_level;
             ...
             (fun () ->
               ...
               let analysis =
                 DDPA_wrapper.create_analysis
                   ~logging_config:(Some ddpa_logging_config) e
               in
               ...
               let check_for_errors () =
                 ...
                 then []
                   ...
                   let module Error_analysis =
                     Toploop_analysis.Make (DDPA_wrapper) in
                   let errors =
               ...
               let standardize_variable_analysis_request () =
                 ...
                 | Analyze_toplevel_variables ->
                     Some
                       (e
                       |> (fun (Expr cls) -> cls)
                       |> List.enum |> Enum.map lift_clause
                       |> Enum.map (fun (Abs_clause (Abs_var (Ident i), _)) ->
                              (i, None, None))
                       |> List.of_enum)
               ...
               let analyze_variable_values requests =
                 ...
                 let varname_to_clause_map =
                   e |> Ast_tools.flatten |> List.map lift_clause
                   |> List.map (fun (Abs_clause (Abs_var i, _) as c) -> (i, c))
                   |> List.enum |> Ident_map.of_enum
                 in
                 (* This utility function helps us use the mapping. *)
                 let lookup_clause_by_ident ident =
                   try Ident_map.find ident varname_to_clause_map
                   with Not_found ->
                     raise
                 ...
                 requests |> List.enum
                 |> Enum.map (fun (var_name, site_name_opt, context_opt) ->
                        ...
                        in
                        callbacks.cb_variable_analysis var_name site_name_opt
                          context_opt values ;
...
let do_evaluation callbacks conf e =
  ...
  then (
    ...
    with Interpreter.Evaluation_failure s ->
      Toploop_types.Evaluation_failure s

let handle_expression ?(callbacks = default_callbacks) conf e =
  try
    ...
    then
      callbacks.cb_source_statistics_callback
      @@ Source_statistics.calculate_statistics e ;
    (* Generate answer. *)
    { illformednesses = []; analyses; errors; evaluation_result }
