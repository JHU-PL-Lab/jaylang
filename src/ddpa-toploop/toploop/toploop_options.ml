...
(** This type defines the configuration required by the toploop to evaluate an
    expression. *)
...
let add_toploop_option_parsers parser =
  ...
  BatOptParse.OptParser.add parser ~long_name:"pdr-deltas"
    pdr_logging_deltas_option ;
  (* Adds control over graph log file name. *)
  BatOptParse.OptParser.add parser ~long_name:"graph-log-file"
    graph_log_file_option ;
  ...
  BatOptParse.OptParser.add parser ~long_name:"disable-analysis" ~short_name:'A'
    disable_analysis_option ;
  (* Add ability to report sizes of generated graphs. *)
  BatOptParse.OptParser.add parser ~long_name:"report-sizes" report_sizes_option ;
...
let read_parsed_toploop_configuration () =
  {
    ...
    topconf_analyze_vars =
      Option.get @@ analyze_variables_option.BatOptParse.Opt.option_get ();
    topconf_disable_evaluation =
      Option.get @@ disable_evaluation_option.BatOptParse.Opt.option_get ();
    topconf_disable_inconsistency_check =
      Option.get
      @@ disable_inconsistency_check_option.BatOptParse.Opt.option_get ();
    topconf_disable_analysis =
      Option.get @@ disable_analysis_option.BatOptParse.Opt.option_get ();
    topconf_report_sizes =
      Option.get @@ report_sizes_option.BatOptParse.Opt.option_get ();
    topconf_report_source_statistics =
      Option.get
