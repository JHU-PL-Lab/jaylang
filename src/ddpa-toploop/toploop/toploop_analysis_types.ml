(** A type utility declaration module. This module is declared separately so its
    types need not be redeclared in an interface. *)
...
let pp_abstract_value_set = Pp_utils.pp_set pp_abstract_value Abs_value_set.enum
and compare_abstract_value_set = Abs_value_set.compare
and equal_abstract_value_set = Abs_value_set.equal

type error =
  | Application_of_non_function of
      abstract_var * abstract_var * abstract_value * abstract_value_set
      (** Represents the application of a non-function value. The arguments are
          the variable identifying the call site clause, the invoked variable,
  ...
  | Projection_of_absent_label of
      ...
      (** Represents the projection of a label from a record value which does
          not have that label. The arguments are the variable identifying the
          clause containing the projection, the record variable, the abstract
          value from which the projection occurred, and the ident we failed to
          project. *)
  | Invalid_binary_operation of
      ...
      * abstract_var
      * abstract_value
      (** Represents invalid use of a binary operator. The arguments are, in
          order,
