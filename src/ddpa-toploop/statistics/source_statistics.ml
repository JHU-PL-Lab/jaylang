...
let ss_join (ss1 : source_statistics) (ss2 : source_statistics) :
    source_statistics =
  {
    ...
    ss_max_lexical_depth = max ss1.ss_max_lexical_depth ss2.ss_max_lexical_depth;
  }

let ss_var_ref (Var (i, _) : var) (db : depth_bindings) (ss : source_statistics)
    : source_statistics =
  let depth = Ident_map.find i db in
  {
    ...
    ss_num_non_local_variable_references_by_depth =
      (let m = ss.ss_num_non_local_variable_references_by_depth in
       if depth = 0
       then m
       else Int_map.add depth (Int_map.find_default 0 depth m + 1) m);
  }
...
let rec calc_expr (Expr cs : expr) (db : depth_bindings) : source_statistics =
  (* Note: we're just binding *every* variable in the expression simultaneously.
     ...
     invalid. *)
  let db' =
    List.fold_left (fun dbacc (Clause (x, _)) -> db_add x dbacc) db cs
  in
  calc_clauses cs db'
...
and calc_clause (Clause (_, b) : clause) (db : depth_bindings) :
  ...
  { ss with ss_num_program_points = ss.ss_num_program_points + 1 }

and calc_clause_body (b : clause_body) (db : depth_bindings) : source_statistics
    =
  match b with
  ...
  | Appl_body (x1, x2) ->
      ...
      { ss with ss_num_function_calls = ss.ss_num_function_calls + 1 }
  | Conditional_body (x, e1, e2) ->
      ss_var_ref x db @@ ss_join (calc_expr e1 db) (calc_expr e2 db)
  | Match_body (x, _) -> ss_var_ref x db empty
...
and calc_value (v : value) (db : depth_bindings) : source_statistics =
  ...
  | Value_record (Record_value m) ->
      { empty with ss_num_variable_references = Ident_map.cardinal m }
  | Value_function f ->
  ...
  | Value_int _ | Value_bool _ -> empty
