...
let heading_var_of_clause = function
  ...
  | Nonbinding_enter_clause
      (_, Abs_clause (Abs_var x, Abs_conditional_body (_, _, _))) ->
      Some x
  | Binding_enter_clause
      (_, _, Abs_clause (Abs_var x, Abs_appl_body (Abs_var _, _))) ->
      Some x
  | _ -> None
...
let all_fun_start_clauses clauses =
  ...
  List.for_all
    (function
      | Binding_enter_clause (Abs_var _, Abs_var _, _) -> true | _ -> false)
    clauses
...
let make_function_mapping e =
  ...
  |> Enum.fold
       (fun (pm, rm, idm) (Function_value (Var (p, _), Expr clss) as f) ->
         ...
         let retvar = clss |> List.last |> fun (Clause (Var (r, _), _)) -> r in
         let rm' = Ident_map.add retvar f rm in
         let idm' = Ident_map.add retvar p idm in
         (pm', rm', idm'))
...
let log_clause c =
  print_endline
  @@ Printf.sprintf "%s\n" (Jhupllib.Pp_utils.pp_to_string pp_clause c)
...
let log_id id =
  print_endline
  @@ Printf.sprintf "%s" (Jhupllib.Pp_utils.pp_to_string pp_ident id)
