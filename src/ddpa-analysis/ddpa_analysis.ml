...
module Make (C : Context_stack) : Analysis_sig with module C = C = struct
  module C = C
  ...
  module Edge_functions =
    Ddpa_pds_edge_functions.Make (C) (Structure_types) (Dynamic_pop_types)
  ...
  let dump_yojson analysis =
    `Assoc
      [
        ("ddpa_graph", Ddpa_graph.to_yojson analysis.ddpa_graph);
        ("ddpa_graph_fully_closed", `Bool analysis.ddpa_graph_fully_closed);
        ( "ddpa_active_nodes",
  ...
  (** Logs a given PDS reachability graph. This only occurs if the logging level
      of the analysis is at least as high as the one provided in this call. The
      graph to be logged defaults to the analysis but can be overridden (e.g. in
      the logger given to that analysis). *)
  let log_pdr level ddpa_logging_data_opt reachability =
    match ddpa_logging_data_opt with
    | None -> ()
    | Some data ->
        if compare_ddpa_logging_level
             data.ddpa_logging_config.ddpa_pdr_logging_level level
           >= 0
        then
          let json =
            `Assoc
              [
                ("element_type", `String "pds_reachability_graph");
                ( "work_count",
                  `Int (Ddpa_pds_reachability.get_work_count reachability) );
                ("graph", Ddpa_pds_reachability.dump_yojson reachability);
              ]
          in
          data.ddpa_logging_config.ddpa_json_logger json

  (** As log_pdr, but logs a delta of the reachability graph. *)
  let log_pdr_delta level ddpa_logging_data_opt old_reachability
      new_reachability =
    match ddpa_logging_data_opt with
    | None -> ()
    | Some data ->
        ...
        then
          let json =
            `Assoc
              [
                ("element_type", `String "pds_reachability_graph_delta");
                ( "work_count",
                  `Int (Ddpa_pds_reachability.get_work_count new_reachability)
                );
                ( "graph",
                  Ddpa_pds_reachability.dump_yojson_delta old_reachability
                    new_reachability );
              ]
          in
          data.ddpa_logging_config.ddpa_json_logger json
  ...
  let log_cfg level analysis =
    ...
    | Some data ->
        if compare_ddpa_logging_level
             data.ddpa_logging_config.ddpa_cfg_logging_level level
           >= 0
        then
          let json =
            `Assoc
              [
                ("element_type", `String "ddpa_graph");
                ( "work_count",
                  `Int
                    (Ddpa_pds_reachability.get_work_count
                       analysis.pds_reachability) );
                ("graph", dump_yojson analysis);
              ]
          in
          data.ddpa_logging_config.ddpa_json_logger json
  ...
  let get_size analysis =
    ...
    let filter_inferrable_nodes nodes =
      nodes
      |> Annotated_clause_set.filter (fun node ->
             match node with
             | Binding_enter_clause _ | Binding_exit_clause _
             | Nonbinding_enter_clause _ ->
                 false
             | _ -> true)
    in
    ( Annotated_clause_set.cardinal
        (filter_inferrable_nodes analysis.ddpa_active_nodes),
      Annotated_clause_set.cardinal
        (filter_inferrable_nodes analysis.ddpa_active_non_immediate_nodes),
      analysis.ddpa_graph |> edges_of |> List.of_enum |> List.length,
      pds_node_count,
      pds_edge_count )
  ...
  let add_edges edges_in analysis =
    let edges =
      edges_in
      |> Enum.filter (fun edge ->
             not @@ Ddpa_graph.has_edge edge analysis.ddpa_graph)
    in
    if Enum.is_empty edges
    then (analysis, false)
    else
      (* ***
      ...
      *)
      ...
      let ddpa_active_nodes', ddpa_active_non_immediate_nodes' =
        ...
        in
        ( Annotated_clause_set.union analysis.ddpa_active_nodes new_active_nodes,
          Annotated_clause_set.union analysis.ddpa_active_non_immediate_nodes
            (new_active_nodes
            |> Annotated_clause_set.filter (not % is_annotated_clause_immediate)
            ) )
      in
      ( {
          ddpa_graph = ddpa_graph';
  ...
  let create_initial_analysis
      ?ddpa_logging_config:(ddpa_logging_config_opt = None) e =
    ...
    let logging_data_opt =
      ...
      | Some config ->
          Some
            {
              ddpa_logging_config = config;
              ddpa_closure_steps =
                0 (* FIXME: this number never changes or gets reported! *);
            }
    in
    (* Lift the expression. *)
    ...
    let rec mk_edges acls' =
      ...
      | Some acl1 -> (
          ...
          | None -> []
          | Some acl2 -> Ddpa_edge (acl1, acl2) :: mk_edges acls')
    in
          ...
          then None
            ...
            Some
              (fun old_reachability new_reachability ->
                ...
                then
                  log_pdr_delta Log_everything logging_data_opt old_reachability
                    new_reachability
                else log_pdr Log_everything logging_data_opt new_reachability)
    in
    ...
    let initial_reachability =
      ...
      |> Ddpa_pds_reachability.add_edge_function (fun state ->
             Enum.singleton
               ([ Pop Structure_types.Bottom_of_stack ], Static_terminus state))
    in
    let empty_analysis =
      {
        ...
        ddpa_active_nodes = Annotated_clause_set.singleton (Start_clause rx);
  ...
  let restricted_values_of_variable x acl ctx analysis =
    ...
    let start_actions = [ Push Bottom_of_stack; Push (Lookup_var x) ] in
  ...
  let contextual_values_of_variable x acl ctx analysis =
    let values, analysis' = restricted_values_of_variable x acl ctx analysis in
    (Abs_value_set.of_enum values, analysis')
  ...
  let perform_closure_steps analysis =
    ...
    | Some data ->
        lazy_logger `trace (fun () ->
            Printf.sprintf "Completed closure step %d" data.ddpa_closure_steps)) ;
