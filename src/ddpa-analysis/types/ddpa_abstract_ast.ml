...
type abstract_var = Abs_var of ident [@@deriving eq, ord, to_yojson, show]

(** A type to expression abstract record values. *)
type abstract_record_value = Abs_record_value of abstract_var Ident_map.t
[@@deriving eq, ord, to_yojson]

(** A type to expression abstract reference values. *)
type abstract_ref_value = Abs_ref_value of abstract_var
[@@deriving eq, ord, to_yojson]
...
and abstract_clause = Abs_clause of abstract_var * abstract_clause_body
[@@deriving eq, ord, to_yojson]

(** A type to represent abstract expressions. *)
and abstract_expr = Abs_expr of abstract_clause list
[@@deriving eq, ord, to_yojson]
...
let rec pp_abstract_function_value formatter (Abs_function_value (x, e)) =
  Format.fprintf formatter "%a -> (@ %a)" pp_abstract_var x pp_abstract_expr e
...
and pp_abstract_value formatter v =
  ...
  | Abs_value_untouched s -> Format.pp_print_string formatter @@ "'" ^ s
  | Abs_value_abort -> Format.pp_print_string formatter "abort"
...
and pp_abstract_clause_body formatter b =
  ...
  | Abs_conditional_body (x, e1, e2) ->
      Format.fprintf formatter "%a @[<4>? @[<2>(%a)@] : @[<2>(%a)@]@]"
        pp_abstract_var x pp_abstract_expr e1 pp_abstract_expr e2
  | Abs_match_body (x, p) ->
  ...
  | Abs_not_body x -> Format.fprintf formatter "not %a" pp_abstract_var x
...
and pp_brief_abstract_record_value formatter (Abs_record_value els) =
  let pp_brief_element formatter (k, v) =
    Format.fprintf formatter "%a=%a" pp_ident k pp_abstract_var v
  in
  pp_concat_sep_delim "{" "}" "," pp_brief_element formatter
  @@ Ident_map.enum els
...
and pp_brief_abstract_clause_body formatter b =
  ...
  | Abs_conditional_body (x, _, _) ->
      Format.fprintf formatter "%a @[<4>? ...@]" pp_abstract_var x
  | Abs_match_body (x, _) ->
      Format.fprintf formatter "%a ~ ..." pp_abstract_var x
  | Abs_projection_body (x, l) ->
      Format.fprintf formatter "%a.%a" pp_abstract_var x pp_ident l
...
and pp_brief_abstract_clause formatter (Abs_clause (x, b)) =
  Format.fprintf formatter "%a = @[<hv 2>%a@]" pp_abstract_var x
    pp_brief_abstract_clause_body b
...
module Annotated_clause = struct
...
module Annotated_clause_set = struct
...
module Annotated_clause_map = struct
...
module Pattern_ord = struct
  type t = pattern

  let compare = compare_pattern
end

module Pattern_set = struct
