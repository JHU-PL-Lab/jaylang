...
module Make
    ...
    struct
      ...
      (** Creates a PDS edge function for a particular DDPA graph edge. The
          resulting function produces transitions for PDS states, essentially
          serving as the first step toward implementing each DDPA rule. The
          remaining steps are addressed by the dynamic pop handler, which
          performs the closure of the dynamic pops generated by this function. *)
      let create_edge_function (edge : ddpa_edge) (state : R.State.t) :
        ...
        Logger_utils.lazy_bracket_log (lazy_logger `trace)
          (fun () ->
            Printf.sprintf "DDPA %s edge function at state %s"
          ...
          (fun edges ->
            let string_of_output (actions, target) =
              String_utils.string_of_tuple
                ...
                R.Terminus.show (actions, target)
            in
        ...
        let targeted_dynamic_pops =
          ...
          @@ List.enum
               [
                 ...
                 (* Value Alias *)
                 (let%orzero (Unannotated_clause
                               (Abs_clause (x, Abs_var_body x'))) =
                    acl1
                  in
                  (* x = x' *)
                  return
                    (Variable_aliasing (x, x'), Program_point_state (acl1, ctx)));
                 (* Stateless Clause Skip *)
                 (let%orzero (Unannotated_clause (Abs_clause (x, _))) = acl1 in
                  (* x' = b *)
                  return
                    (Nonmatching_clause_skip x, Program_point_state (acl1, ctx)));
                 (* Block Marker Skip *)
                 (* This is handled below as a special case because it does not involve
                    a pop. *)
                 (* ********** Navigation ********** *)
                 (* Capture *)
                 return (Value_capture_1_of_3, Program_point_state (acl0, ctx));
                 (* ********** Function Wiring ********** *)
                 ...
                 (let%orzero (Binding_enter_clause (x, x', c)) = acl1 in
                  ...
                  if not (equal_abstract_var x' x3'')
                  then
                 ...
                 (let%orzero (Binding_exit_clause (x, _, c)) = acl1 in
                  let%orzero (Abs_clause (x1'', Abs_appl_body (x2'', x3''))) =
                    c
                  in
                  if not (equal_abstract_var x x1'')
                  then
                    raise @@ Utils.Invariant_failure "Ill-formed wiring node."
                  else () ;
                  (* x =(up)c _ (for functions) *)
                  return
                    ( Function_call_flow_validation
                        (x2'', x3'', acl0, ctx, Unannotated_clause c, ctx, x),
                 ...
                 (let%orzero (Binding_exit_clause (x, x', c)) = acl1 in
                  let%orzero (Abs_clause (x2, Abs_conditional_body (x1, e1, _)))
                      =
                    c
                  in
                  if not (equal_abstract_var x x2)
                  then
                    raise @@ Utils.Invariant_failure "Ill-formed wiring node."
                  else () ;
                  ...
                  return
                    ( Conditional_subject_evaluation
                        (x, x', x1, then_branch, acl1, ctx),
                      Program_point_state (Unannotated_clause c, ctx) ));
                 (* ********** Pattern Matching ********** *)
                 (* Pattern Matching Start *)
                 (let%orzero (Unannotated_clause
                               (Abs_clause (x1, Abs_match_body (x2, p)))) =
                    acl1
                  in
                  (* x1 = x2 ~ p *)
                  return
                 ...
                 return (Matching_1_of_2, Program_point_state (acl0, ctx));
                 (* ********** Record Projection ********** *)
                 (* Record Projection Start *)
                 (let%orzero (Unannotated_clause
                               (Abs_clause (x1, Abs_projection_body (x2, lbl))))
                      =
                    acl1
                  in
                  (* x1 = x2.lbl *)
                  return
                    ( Record_projection_lookup (x1, x2, lbl),
                      Program_point_state (acl1, ctx) ));
                 (* Record Projection Stop *)
                 return
                   (Record_projection_1_of_2, Program_point_state (acl0, ctx));
                 (* ********** Operations ********** *)
                 (* Not Start *)
                 (let%orzero (Unannotated_clause
                               (Abs_clause (x1, Abs_not_body x2))) =
                    acl1
                  in
                  (* x1 = not x2 *)
                  return (Not_lookup (x1, x2), Program_point_state (acl1, ctx)));
                 (* Not Evaluation *)
                 return (Not_resolution, Program_point_state (acl0, ctx));
                 (* Binary Operation Start *)
                 (let%orzero (Unannotated_clause
                               (Abs_clause
                                 (x1, Abs_binary_operation_body (x2, _, x3)))) =
                    acl1
                  in
                  (* x1 = x2 op x3 *)
                  return
                    ( Binary_operator_lookup_init
                        (x1, x2, x3, acl1, ctx, acl0, ctx),
                      Program_point_state (acl1, ctx) ));
                 (* Binary Operation Evaluation *)
                 (let%orzero (Unannotated_clause
                               (Abs_clause
                                 (x1, Abs_binary_operation_body (_, op, _)))) =
                    acl1
                  in
                  (* x1 = x2 op x3 *)
                  return
        ...
        Enum.append
          ...
          |> Enum.map (fun (action, state) ->
                 ([ Pop_dynamic_targeted action ], Static_terminus state)))
          (nop_states |> Enum.map (fun state -> ([], Static_terminus state)))

      let create_untargeted_dynamic_pop_action_function (_ : ddpa_edge)
          (_ : R.State.t) =
