...
module Make (C : Context_stack) = struct
  ...
  module Bounded_capture_size : Bounded_capture_size_sig = struct
    ...
    let pp formatter (Bounded_capture_size n) = Format.pp_print_int formatter n
  ...
  type pds_continuation =
    | Bottom_of_stack
        (** The bottom of stack element is necessary as a sentinel. It's pushed
            as the initial element on the continuation stack so we don't need to
            check for empty continuation stacks. *)
  ...
  module Pds_continuation = struct
