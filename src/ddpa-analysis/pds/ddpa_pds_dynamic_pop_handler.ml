    ...
    struct
      ...
      let perform_targeted_dynamic_pop element action =
        ...
        | Value_capture_1_of_3 ->
            ...
            return
              [ Pop_dynamic_targeted (Value_capture_2_of_3 abs_filtered_value) ]
        | Value_capture_2_of_3 fv ->
            let%orzero (Capture size) = element in
            return
              [ Pop_dynamic_targeted (Value_capture_3_of_3 (fv, [], size)) ]
              ...
              return
                [
                  Pop_dynamic_targeted
                    (Value_capture_3_of_3
                       (fv, element :: collected_elements, size'));
                ]
            else
              let pushes =
        ...
        | Function_call_flow_validation_resolution_2_of_2 (x, x') ->
            let%orzero (Continuation_value v) = element in
            let%orzero (Abs_value_function
                         (Abs_function_value (_, Abs_expr acls))) =
              v
            in
        ...
        | Function_closure_lookup (x'', xf) ->
            ...
            (* We're looking for a non-local variable.  Push a lookup for the
               function. *)
            return [ Push element; Push (Lookup_var xf) ]
        ...
        | Matching_lookup (x1, x2, p) ->
            ...
            return [ Push (Continuation_pattern p); Push (Lookup_var x2) ]
        | Matching_1_of_2 ->
            let%orzero (Continuation_value v) = element in
            return [ Pop_dynamic_targeted (Matching_2_of_2 v) ]
        | Matching_2_of_2 v1 ->
            let%orzero (Continuation_pattern p) = element in
            let%orzero (Some pattern_match_res) = abstract_pattern_match v1 p in
            let%bind v2 = pick_enum @@ pattern_match_res in
            return [ Push (Continuation_value v2) ]
        | Record_projection_lookup (x, x', l) ->
            let%orzero (Lookup_var x0) = element in
            [%guard equal_abstract_var x0 x] ;
            return [ Push (Project l); Push (Lookup_var x') ]
        ...
        | Not_lookup (x1, x2) ->
            let%orzero (Lookup_var x1') = element in
            [%guard equal_abstract_var x1 x1'] ;
            return [ Push (Lookup_var x2) ]
        | Not_resolution ->
            let%orzero (Continuation_value v) = element in
            let%orzero (Some vrs) = abstract_not v in
            let%bind vr = pick_enum vrs in
            return [ Push (Continuation_value vr) ]
        | Binary_operator_lookup_init (x1, x2, x3, acl1, ctx1, acl0, ctx0) ->
            ...
            let k1'' = [ Capture capture_size_5; Lookup_var x2 ] in
            let k2'' =
              [ Capture capture_size_2; Lookup_var x3; Jump (acl1, ctx1) ]
            in
        ...
        | Binary_operator_resolution_3_of_4 (x1, op, v2) ->
            ...
            let%orzero (Some result_values) =
              abstract_binary_operation op v1 v2
            in
      ...
      let perform_untargeted_dynamic_pop element action =
        ...
        | Value_discovery_1_of_2 ->
            let%orzero (Continuation_value v) = element in
            return
              ( [ Pop_dynamic_targeted Value_discovery_2_of_2 ],
