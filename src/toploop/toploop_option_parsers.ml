  ...
  in
  {
    option_set =
      (fun option_name args ->
        ...
        | [ analysis_name ] ->
            let analysis_module =
              try Toploop_utils.stack_from_name analysis_name
              with Not_found ->
                raise
                @@ Option_error
                     ( option_name,
...
let ddpa_logging_level_option name =
  let logging_level = ref None in
  {
        ...
        | [ level_name ] ->
            let level =
              ...
              | _ ->
                  ...
                  @@ Option_error
                       ( option_name,
                         Printf.sprintf "Invalid %s logging level: %s" name
                           level_name )
            in
    ...
    option_defhelp =
      Some ("Selects a " ^ name ^ " logging level (none,result,all).");
  }
...
let pdr_logging_deltas_option =
  {
    (BatOptParse.StdOpt.store_true ()) with
    option_defhelp = Some "Logs reachability graphs as deltas to save space.";
  }

let graph_log_file_option =
  {
    (BatOptParse.StdOpt.str_option ~default:"ddpa_graphs.log.json"
       ~metavar:"GRAPH_LOG_FILE" ())
    with
    option_defhelp = Some "Specifies the name for the DDPA graph log file.";
  }
...
let analyze_variables_option =
  let variables_to_analyze = ref Analyze_no_variables in
  {
      ...
      (fun option_name args ->
        ...
        | [ analyze_string ] ->
            let new_selection =
              ...
              | _ ->
                  if String.starts_with analyze_string "only:" then
                    let components =
                      analyze_string |> String.lchop ~n:5 |> fun x ->
                      String.split_on_string x ~by:","
                    in
                      ...
                      | [ name; rest ] -> (
                          ...
                          | _ ->
                              ...
                              @@ Option_error
                                   ( option_name,
                                     Printf.sprintf
                                       "Invalid component string: %s" component
                                   ))
                      | _ ->
                          raise
                          @@ Option_error
                    ...
                    Analyze_specific_variables
                      (List.map parse_component components)
                  else
                    raise
                    @@ Option_error
                         ( option_name,
                           Printf.sprintf
                             "Unrecognized variable analysis mode: %s"
                             analyze_string )
            in
    ...
    option_defhelp =
      Some
        "Selects variables to analyze.  Valid options are \"none\" (to perform \
         no variable-specific analysis), \"top\" (to analyze all top-level \
         variables from the end of the program), or \"only\".  If \"only\" is \
         given, it must be followed by a colon and then a comma-separated list \
         of specifications.  A specification is the name of a variable (e.g. \
         \"a\"), optionally followed by \"@\" and a lookup site variable (e.g. \
         \"s\"), optionally followed by \":\" and a pipe-separated list of \
         variable names representing a context stack (from top to bottom).";
  }

let disable_evaluation_option = BatOptParse.StdOpt.store_true ()

let disable_inconsistency_check_option = BatOptParse.StdOpt.store_true ()

let disable_analysis_option = BatOptParse.StdOpt.store_true ()

let report_sizes_option = BatOptParse.StdOpt.store_true ()

let report_source_statistics_option = BatOptParse.StdOpt.store_true ()
