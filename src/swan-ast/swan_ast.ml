...
and pp_swan_var : Format.formatter -> swan_var -> Ppx_deriving_runtime.unit=
  let __1 () = pp_ident
  and __0 () = pp_uid in
  ((let open! Ppx_deriving_runtime in
      fun fmt  ->
        function
        | Swan_var (a0,a1) ->
            (Format.fprintf fmt "@[<hov2>Swan_ast.Swan_var (@,";
             (((__0 ()) fmt) a0; Format.fprintf fmt ",@ "; ((__1 ()) fmt) a1);
             Format.fprintf fmt "@])"))[@ocaml.warning "-A"])
and show_swan_var : swan_var -> Ppx_deriving_runtime.string=
  fun x  -> Format.asprintf "%a" pp_swan_var x
