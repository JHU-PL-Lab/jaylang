...
let test_exact_expected testname _args =
  let result =
    match
      ...
      File_utils.load_expect testname @@ List.t_of_sexp Branch_info.t_of_sexp
    with
    | None -> false (* no expected output, so say it failed *)
    | Some expect_list ->
        let actual_output =
          Concolic.eval ~quit_on_abort:false
            (Dj_common.File_utils.read_source testname)
        in
        List.exists expect_list ~f:(fun e ->
            Branch_info.compare actual_output e = 0)
  in
...
let test_for_abort testname _args =
  ...
  (* existence of this file implies an abort should be found *)
  ...
  |> (function
       | Concolic.Test_result.Timeout | Exhausted | Exhausted_pruned_tree ->
           false (* did not find error *)
...
let () =
  ...
  let _bjy_tests =
    From_lib.group_tests "test-sources/concolic/_bjy_tests/" `Slow
      test_for_abort
  in
  let bjy_tests =
    From_lib.group_tests "test-sources/concolic/bjy_tests/" `Quick
      test_for_abort
  in
  Alcotest.run_with_args "concolic" Test_argparse.config
    (bjy_tests @ _bjy_tests @ grouped_tests)
    ~quick_only:false
